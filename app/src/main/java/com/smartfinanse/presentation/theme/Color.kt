package com.smartfinanse.presentation.theme

import androidx.compose.ui.graphics.Color

// Brand — "Spokojne finanse"
val GreenPrimary = Color(0xFF1B5E20)
val GreenLight = Color(0xFF4CAF50)
val GreenContainer = Color(0xFFE8F5E9)
val GreenDark = Color(0xFF003910)
val GreenDarkContainer = Color(0xFF2E7D32)

// Neutral backgrounds
val BackgroundLight = Color(0xFFF7F9F8)
val BackgroundDark = Color(0xFF121212)

val SurfaceLight = Color(0xFFFFFFFF)
val SurfaceDark = Color(0xFF1E1E1E)

// Subtle accent only on dashboard chart card (not global mint chrome)
val ChartCardLight = Color(0xFFE8F5E9)
val ChartCardDark = Color(0xFF1A2E1C)

// Neutrals
val OnSurfaceLight = Color(0xFF1C1B1F)
val OnSurfaceDark = Color(0xFFE6E1E5)
val OnSurfaceVariantLight = Color(0xFF49454F)
val OnSurfaceVariantDark = Color(0xFFCAC4D0)
val SurfaceVariantLight = Color(0xFFEEF1F0)
val SurfaceVariantDark = Color(0xFF2C2C2C)
val OutlineLight = Color(0xFF79747E)
val OutlineDark = Color(0xFF938F99)

// Error (Material 3 baseline)
val ErrorLight = Color(0xFFB3261E)
val OnErrorLight = Color(0xFFFFFFFF)
val ErrorContainerLight = Color(0xFFF9DEDC)
val OnErrorContainerLight = Color(0xFF410E0B)

val ErrorDark = Color(0xFFF2B8B5)
val OnErrorDark = Color(0xFF601410)
val ErrorContainerDark = Color(0xFF8C1D18)
val OnErrorContainerDark = Color(0xFFF9DEDC)
